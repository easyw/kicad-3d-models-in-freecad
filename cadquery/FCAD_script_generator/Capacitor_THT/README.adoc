...
.REQUIREMENTS
* FreeCAD 0.17 (for geometry check)
* CadQuery 0.5.2
...
freecad ./main_generator.py [optional paramemters] +
examples: +
+[freecad bin] main_generator.py ?+ +
+[freecad bin] main_generator.py series=axial_tht+ +
+[freecad bin] main_generator.py series=axial_tht filter=*D6.5*+ +
+[freecad bin] main_generator.py+ +

[freecad bin]: this is operating system dependend. +
+for windows: c:\freecad\bin\freecad (depends on installation settings)+ +
+for linux: freecad, freecad-daily or FreeCAD+ +


modelName and parameters are described in: +
1. *_params.py files. All data in parameter files are collected from components data sheets +
##the script will generate STEP and VRML parametric models##
ready to be used with kicad StepUp script (and kicad internal 3d viewer.)
