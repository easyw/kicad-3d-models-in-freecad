...
class cq_parameters_ProjectsUnlimited_AI_4228_TWT_R():
...
#        case = case.faces("<Z").shell(0.3)
    ...
    def make_case(self, params):
        ...
        case = cq.Workplane("XY").workplane(offset=A1).moveTo(x, y).circle(d1, False).extrude(1.0)
        #
        case1 = cq.Workplane("XY").workplane(offset=A1).moveTo(x, y).circle(d1, False).extrude(H)
        case2 = cq.Workplane("XY").workplane(offset=A1 + 8.0).moveTo(x, y).circle(d1 - 5.0, False).extrude(H)
        case1 = case1.cut(case2)
        case1 = case1.faces(">Z").edges(">Y").fillet(0.98)
        case = case.union(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=A1).moveTo(x, y).circle(d1 - 7.0, False).extrude(H)
        case2 = cq.Workplane("XY").workplane(offset=A1 + 8.0).moveTo(x, y).circle(d1 - 8.0, False).extrude(H)
        case1 = case1.cut(case2)
        case1 = case1.faces(">Z").edges(">Y").fillet(0.48)
        case = case.union(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=A1).moveTo(x, y).circle(d1 - 10.0, False).extrude(H)
        case2 = cq.Workplane("XY").workplane(offset=A1 + 8.0).moveTo(x, y).circle(d1 - 11.0, False).extrude(H - 3.0)
        case1 = case1.cut(case2)
        case1 = case1.faces(">Z").edges(">Y").fillet(0.48)
        case = case.union(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=A1 + 3.0).moveTo(x, y).circle(d1 - 13.0, False).extrude(H - 3.0)
        case1 = case1.faces(">Z").edges(">Y").fillet(0.48)
        case = case.union(case1)

#        case = case.faces(">Z").edges(">Y").fillet(0.5)
        case = case.faces("<Z").edges(">Y").fillet(D / 50.0)

        case1 = cq.Workplane("XY").workplane(offset=A1 + (H / 5.0)).moveTo(x, y).circle(3.5, False).extrude(H + (3.0 * (H / 5.0)))
        case = case.cut(case1)
#        case = case.faces("<Z").shell(0.3)
        ...
        # Add the '+'
        #
        case1 = cq.Workplane("XY").workplane(offset=A1 + H - 0.2).moveTo((x - (D / 2.0)) + 2.5, 0).rect(2.0, 0.5).extrude(0.4)
        case = case.cut(case1)
        case1 = cq.Workplane("XY").workplane(offset=A1 + H - 0.2).moveTo((x - (D / 2.0)) + 2.5, 0).rect(0.5, 2.0).extrude(0.4)
        case = case.cut(case1)
    ...
    all_params = {
        ...
        'MagneticBuzzer_ProjectsUnlimited_AI-4228-TWT-R': Params(
            #
            # Valve
            ...
            # 
            # The foot print that uses this 3D model is MagneticBuzzer_ProjectsUnlimited_AI-4228-TWT-R.kicad_mod
            # 
            modelName = 'MagneticBuzzer_ProjectsUnlimited_AI-4228-TWT-R',   # modelName
            D = 42.00,                  # Body width/diameter
            H = 14.00,                  # Body height
            A1 = 0.03,                  # Body-board separation
            b = 1.30,                   # Pin diameter
            center = (12.70, 0.00),     # Body center
            ph = 6.5,                   # Pin length
            pin = [(0.0, 0.0), (25.40, 0.00)],          # Pins
