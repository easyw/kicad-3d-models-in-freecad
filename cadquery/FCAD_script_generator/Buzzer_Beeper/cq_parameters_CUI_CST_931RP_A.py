...
class cq_parameters_CUI_CST_931RP_A():
    ...
    def make_case(self, params):
        ...
        x = center[0]
        y = center[1]
        case = cq.Workplane("XY").workplane(offset=A1).moveTo(x, 0 - y).circle(D / 2.0, False).extrude(H)
        #
        # Bottom cut
        #
        case1 = cq.Workplane("XY").workplane(offset=A1).moveTo(0.0, 0 - y).rect(3.0, 12.0).extrude(0.5)
        case = case.cut(case1)
        #
        # Bottom cut
        #
        case1 = cq.Workplane("XY").workplane(offset=A1).moveTo(0.0, 0 - y).rect(12.0, 3.0).extrude(0.5)
        case = case.cut(case1)
        #
#        case1 = cq.Workplane("XY").workplane(offset=A1).moveTo(x, 0 - y).circle((D / 2.0) - 0.4, False).extrude(0.5)
#        case = case.cut(case1)
        #
        # Add pipe
        #
        case1 = cq.Workplane("XY").workplane(offset=A1 + H - 2.5).moveTo(0.0, y + 1.0).rect(5.0, 2.5).extrude(2.5)
        case = case.union(case1)
        #
        # Cut hole in pipe
        #
        case1 = cq.Workplane("XY").workplane(offset=A1 + H - 1.0).moveTo(0.0, y + 1.0).rect(3.6, 2.5).extrude(0.7)
        case = case.cut(case1)
        #
...
#        case = case.faces("<Z").shell(0.3)
    ...
    def make_pins(self, params):
        ...
        b1 = b * 0.9
        p = pin[0]
        pins = cq.Workplane("XY").workplane(offset=A1 + 1.0).moveTo(p[0], 0 - p[1]).circle(b1 / 2.0, False).extrude(0 - (ph + A1 + 1.0))
        pins = pins.faces("<Z").fillet(b / 5.0)
        
        for i in range(1, len(pin)):
            p = pin[i]
            pint = cq.Workplane("XY").workplane(offset=A1 + 1.0).moveTo(p[0], 0 - p[1]).circle(b1 / 2.0, False).extrude(0 - (ph + A1 + 1.0))
