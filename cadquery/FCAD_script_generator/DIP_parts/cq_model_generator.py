...
class ModelGenerator:
    ...
    def __init__(self, scripts_root, script_dir=None, saveToKicad=True, kicadStepUp=None):
        ...
        if self.script_dir == "" or self.script_dir == None:
            self.script_dir = None
        self.models_dir = os.getenv('KISYS3DMOD') if saveToKicad else None
        if self.models_dir == None:
        ...
        if not os.path.isdir(self.footprints_dir):
    ...
    def setScriptSource(self, url):
        ...
        """
        self.scriptsource = "" if url == None else url
    ...
    def makeModel(self, models_dir, genericName, model, keepDocument=True, verbose=False):
        ...
        license_txt = list(add_license.LIST_int_license if self.license == None else self.license) # make a copy to avoid modifying the original
        license_txt.append("")
        license_txt.append("")
        if self.scriptsource != "" and self.script_dir != None:
            FreeCAD.Console.PrintMessage("\nAppn")
        ...
        if len(options) > 0 and not params.base_params.has_key(options[0]):
                ...
                for variant in models.keys():
                    if qfilter.match(variant):
                        ...
                        if (buildAllSMD == False or params.type == CaseType.SMD) and model.make_me:
                            models_made = models_made + 1
                            self.makeModel(self.models_dir, variant, model, False)
        else:
            ...
            if family == All:
                ...
                for variant in models.keys():
                    ...
                    if model.make_me:
                        models_made = models_made + 1
                        self.makeModel(self.models_dir, variant, model, True)
                    else:
