...
__title__ = "make Phoenix PT terminal block models"
__author__ = "maurice, hyOzd and grob"
__Comment__ = 'make Phoenix PT terminal block 3D models exported to STEP and VRML for Kicad StepUP script'

___ver___ = "0.1.0 2019-05-09"
...
#body_color_key = "light green body"
...
#except:
  #  try:
from FreeCADGui.Command import *
...
all_params = kicad_naming_params_phoenix_pt
...
def MakeTerminalBody(params):
    # some parameters (maybe move to params at some point)
    backchamfer_y = 0.55
    backchamfer_z = 4.20
    backchamferinset_y = 0.10
    frontchamger_y = 0.55
    frontchamfer_z = 4.20
    frontchamferinset_y = 0.20
    
    # some helper values
    x0 = -1.0*params.p/2.0 # start position for terminal
    l = params.p * params.n
    
    # body prism
    body = cq.Workplane("YZ").rect(params.y,params.h,False).extrude(l)
    body = body.faces("<X").vertices("<YZ").workplane().center(0,params.h).lineTo(backchamfer_y+backchamferinset_y, params.h).lineTo(backchamferinset_y,params.h-backchamfer_z).lineTo(0,params.h-backchamfer_z).lineTo(0,params.h).cutThruAll()
    body = body.faces("<X").vertices("<YZ").workplane().center(params.y,params.h).lineTo(params.y-frontchamfer_y-frontchamferinset_y, params.h).lineTo(params.y-frontchamferinset_y,params.h-frontchamfer_z).lineTo(params.y,params.h-frontchamfer_z).lineTo(params.y,params.h).cutThruAll()
...
def MakePart(params, name, n=1):
    ...
    body_output = MakeTerminalBody(params)
    ...
    # different parts have different colours. use params value to pick!
    body_color_key = params.color + " body"
    body_color = shaderColors.named_colors[body_color_key].getDiffuseFloat()
...
if __name__ == "__main__" or __name__ == "main_generator":
    ...
    if len(sys.argv) < 3:
        ...
        modelnames = ['TerminalBlock_Phoenix_PT-1,5-2-5.0-H_1x02_P5.00mm_Horizontal']
    else:
        if (sys.argv[2] == 'all'):
