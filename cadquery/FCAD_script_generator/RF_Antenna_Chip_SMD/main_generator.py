...
try:
    ...
    import importlib
    importlib.reload(cq_cad_tools)# Explicitly load all needed functions
...
def make_chip(model, all_params):
    ...
    pm = all_params[model]['pm'] # polarity mark
    ...
    pinmark = cq.Workplane(cq.Plane.XY()).workplane(offset=width*0.01).rect(width/2, pm). \
                workplane(offset=height*0.01).rect(width/2, pm). \
                loft(ruled=True)
