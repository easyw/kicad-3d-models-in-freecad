Till few time ago, yaml was not included on FC Appimage and adding yaml is not easy on Appimages.
Since latest FC 0.19 yaml has been added, so it won't be an issue for next FC releases, but it could for version 0.18 and older.
...
examples in win: +
+c:\freecad\bin\freecad main_generator.py R_0402+ +
