...
import cq_params_diode  # modules parameters
from cq_params_diode import *

all_params= all_params_diode

def make_diode(params):
    pml = params.pml 
    c  = params.c  # pin thickness, body center part height
    the  = params.the  # body angle in degrees
    tb_s  = params.tb_s  # top part of body is that much smaller
    ef  = params.ef  # 0.05,      # fillet of edges  Note: bigger bytes model with fillet
    cc1 = params.cc1  # 0.45 chamfer of the 1st pin corner
    ...
    R1  = params.R1  # pin upper corner, inner radius
    R2  = params.R2  # pin lower corner, inner radius
    S  = params.S   # pin top flat part length (excluding corner arc)
# automatically calculated    L  = params.L
    D1  = params.D1  # body length
    E1  = params.E1  # body width
    E   = params.E  # body overall width  E=E1+2*(S+L+c)
    A1  = params.A1  # body-board separation
    A2  = params.A2  # body height
    ...
    pinmark = cq.Workplane(cq.Plane.XY()).workplane(offset=A2-0.0013).rect(E1_t2, pml). \
                workplane(offset=0.01).rect(D1_t2, pml). \
                loft(ruled=True)
