...
class cq_littlefuse():
    ...
    def make_top(self, modelID):
        ...
        # Make dummy
        #
        case = cq.Workplane("XY").workplane(offset=2.0).moveTo(0.0, 0.0).circle(0.01 , False).extrude(0.01)
    ...
    def make_body_Littelfuse_No560_No460(self, modelID):
        ...
        case = cq.Workplane("XY").workplane(offset=tdz).moveTo(0.0, 0.0).circle(W / 2.0).extrude(H)
        #
        # Add pigs under
        #
        case1 = cq.Workplane("XY").workplane(offset=0.0).moveTo(0.0, W / 4.0).circle(0.7).extrude(tdz)
        case = case.union(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=0.0).moveTo(0.0, 0.0 - (W / 4.0)).circle(0.7).extrude(tdz)
        case = case.union(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=0.0).moveTo((W / 2.0) - 0.5, 0.0).circle(0.5).extrude(tdz)
        case = case.union(case1)
        #
        case1 = cq.Workplane("XY").workplane(offset=0.0).moveTo(0.0 - ((W / 2.0) - 0.5), 0.0).circle(0.5).extrude(tdz)
        case = case.union(case1)
        #
        #
        dh = W / 2.0
        ...
        dy = dh / (sqrt(2.0))
        #
        case1 = cq.Workplane("XY").workplane(offset=0.0).moveTo(dx, 0.0 - dy).circle(0.75).extrude(H + tdz)
        case = case.union(case1)
        #
        #
        #
        ...
        for p in pin:
            ...
            l = p[4]
            h = p[5]
            case1 = cq.Workplane("XY").workplane(offset=(H / 2.0) + tdz).moveTo(x, y).circle(1.0).extrude(H)
            case = case.cut(case1)
    ...
    def make_pin_Littelfuse_No560_No460(self, modelID):
        ...
        for p in pin:
            ...
            if p[0] == 'rect':
                ...
                case1 = cq.Workplane("XY").workplane(offset=0.0).moveTo(x, y).circle(w, False).extrude(0.0 - h)
                case1 = case1.faces("<Z").fillet(w / 2.2)
                case2 = cq.Workplane("XY").workplane(offset=0.0).moveTo(x, y).circle(1.0).extrude((H + H1))
                case2 = case2.faces("<Z").chamfer(w, w)
                case1 = case1.union(case2)
                case2 = cq.Workplane("XY").workplane(offset=(H / 2.0) + H1).moveTo(x, y).circle(0.5).extrude(H)
    ...
    def make_npth_pin(self, modelID):
        ...
        if npthpin == None:
            case = cq.Workplane("XY").workplane(offset=2.0).moveTo(0.0, 0.0).circle(0.001, False).extrude(0.001)
        else:
            for n in npthpin:
                t = n[0]
                x = n[1]
